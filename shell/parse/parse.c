...
t_token *merge_token(t_token *head)
{
    ...
    while (current && current->next)
    {
        ...
        {
            printf("DEBUG: Merge öncesi - Token1: '%s' type:%d, Token2: '%s' type:%d\n", 
                current->value, current->type, current->next->value, current->next->type);
            t_token *next_token = current->next;
            char *merged_value = ft_strjoin(current->value, next_token->value);
            current ->type = WORD;
            printf("DEBUG: Merge sonrası - Token: '%s' type:%d\n", 
                   merged_value, current->type);
            free(current->value);
