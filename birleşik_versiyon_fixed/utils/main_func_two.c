...
// INVALID token'a kadar olan tüm token'ları sil
...
// Command işlemlerini process et
int process_commands(t_shell_state *state)
{
    // Val zaten init_shell_state'de oluşturulmuş, burada yeniden oluşturmaya gerek yok
    ...
    // INVALID token varsa önceki token'ları sil (merge'den sonra)
...
// Execute işlemini handle et
void handle_execution(t_shell_state *state, char **envp)
{
    state->env_status = set_environment(state->tokens, &state->expansion, state->equal_status);
    
    if (state->equal_status != VALID && !state->token_check && state->env_status == 0)
