...
{
    /* Apply heredoc first (may be overridden by later input redirs) */
    ...
    /* Apply redirections in left-to-right order if present */
    ...
    else if (cmd->output_file && redirect_output(cmd->output_file, cmd->append) < 0)
    {
        fd_gc_cleanup();
        gc_cleanup();
        _exit(1);
