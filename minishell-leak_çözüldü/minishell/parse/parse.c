...
t_token	*tokenize(const char *input)
{
	...
	while (*p)
	{
		...
		if (*p)
		{
			...
			if (!val)
			{
				// No need to manually free tokens as GC will handle it
				return (NULL);
			}
...
static void	merge_current_with_next(t_token *current)
{
	...
	if(current->type == SINGLE_QUOTE || next_token->type == SINGLE_QUOTE)
		...
		current->expandable_fd = 1;
	current->type = WORD;
	current->value = merged_value; // Old value will be cleaned by GC
	...
	// No need to free next_token and its value as GC will handle it
}
