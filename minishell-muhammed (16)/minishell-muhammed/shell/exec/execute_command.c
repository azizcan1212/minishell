	...
	get_heredoc_fd(const char *delim, t_shell_val *val, int expandable_fd)
{
	...
	{
		close(p[0]);
		/* custom handler so we can cleanup instead of default killing without gc */
...
int	execute_command(t_command *head, char **envp, t_shell_val *val)
{
	...
	// Debug: execute_command start
		(void)head;
	while (node)
	{
		// Debug: processing node
			/* processing each command node */
		node->heredoc_fd = -1;
		if (node->heredoc_list)
		{
			...
			while (h)
			{
				...
				{
					heredoc_interrupted = 1; // only heredoc SIGINT should abort execution phase
					break;
				}
				if (!h->next)
	...
	// After heredoc preparation, print head summary
	if (head)
			(void)head;
	...
	else if (val->last_exit_status == 130)
	{
		// Ctrl-C at prompt sets 130 but should NOT block next command execution
			/* previous prompt interrupt; continue */
	}
	if (head && !head->next && (!head->cmd || head->cmd[0] == '\0'))
	{
			/* empty single command: only redirs handled */
		if (head->input_file)
		{
	...
	// Debug: before builtin check
		/* check builtin in parent */
	if (head && !head->next
		&& (ret = handle_builtin_commands(head, val, envp)) != -1)
	{
		// Debug: builtin handled
			/* builtin handled */
		return (val->last_exit_status = ret, ret);
	}
		/* forking for external / pipeline */
